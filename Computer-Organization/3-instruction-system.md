...
+ 一条指令的执行分为三步，四次访存：
  ...
  2. 取两个操作数。
  3. 放回结果。
+ 访存是指访问内存，$ACC$在运算器中，访问$ACC$不是访问内存。
...
+ 但至少须将$1111$留作扩展操作码之用，即三地址指今为$15$条。所以目前就留下来了前$4$位操作数为$1111$的指令，这是只有四位操作码。
...
假设地址长度为$n$，上一层留出$m$种状态，则下一层可以拓展出$m\times2^n$种状态。
...
+ 大端方式：字地址为高字节地址。如$012345$。
+ 小端方式：字地址为低字节地址。如$452301$。
...
+ 顺序寻址：由$PC$自动加上某个数寻址下一个要操作的指令，一般是加一个指令字长。
+ 跳跃寻址：由转移指令指出。如执行到操作码为$JMP$，则跳跃到保存的地址码所指向的指令地址进行执行。跳跃地址分为绝对地址（标记符直接得到）和相对地址（相对当前地址的偏移量）。跳跃的结构是当前指令修改$PC$值，所以下一条指令仍通过$PC$给出。
