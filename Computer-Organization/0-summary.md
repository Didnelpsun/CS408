...
1. 汇编指令：与机器指令一一对应，它是机器码的助记符，汇编后变成机器语言可以被计算机执行。
2. 伪指令：由编译器识别执行，用于指示汇编程序如何汇编源程序，也称为命令语句，编译结束后伪指令失去作用，计算机不执行。
...
有$16$根数据线和$20$根地址线，它既能处理$16$位数据，也能处理$8$位数据，可以存放两个字节，其中高位为$\cdos H$，低位为$\cdots L$，如$AX$分为两个八位的$AH$和$AL$。可寻址的内存空间为$2^{20}=1MB$。

### 寄存器

$8086CPU$有$14$个$16$位寄存器，分别为：

+ AX、BX、CX、DX：通用寄存器，用来存放一般性数据。
+ SI。
+ DI。
+ SP。
+ BP。
